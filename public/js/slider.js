...
AFRAME.registerComponent("slider", {
  ...
  onControllerConnected: function() {
    ...
    pickerBackPlaneEl = this.pickerBackPlaneEl = document.createElement(
      "a-entity"
    );
  ...
  tick: function() {
    ...
    if (!this.el.sceneEl.is("vr-mode") || !handEl) {
      return;
    }
    intersection = handEl.components.raycaster.getIntersection(
      pickerBackPlaneEl
    );
    if (!intersection) {
    ...
    evtDetail.value =
      (pickerHitBoxEl.object3D.position.x + halfWidth) / this.data.width;
