(function() {
  ...
  function init() {
    ...
    var sceneObjects = sceneElements
      .filter(
        element =>
    ...
    // Define prompts.
    const WelcomePrompt =
      "Hello. Do you want me to tell you about this WebXR scene? ";
    ...
    const HelpPrompt =
      "You can ask me to describe the scene, ask me how many things there are or what colour something is, or tell me to stop when you've had enough. What do you want me to tell you? ";

    // Check for SpeechRecognition support.
    if (!("webkitSpeechRecognition" in window)) {
      alert(
        "This browser does not support the Web Speech API SpeechRecognition interface."
      );
    } else {
    ...
    if (window.SpeechSynthesisUtterance === undefined) {
      alert(
        "This browser does not support the Web Speech API SpeechSynthesis interface."
      );
    } else {
    ...
    recognition.onresult = function(event) {
      ...
      for (let i = event.resultIndex; i < event.results.length; ++i) {
        if (event.results[i].isFinal) {
          ...
          if (
            transcriptBits.includes("yes") ||
            transcriptBits.includes("describe") ||
            transcriptBits.includes("tell me about")
          ) {
            describeScene();
            ...
            transcriptBits.includes("colour") ||
            transcriptBits.includes("color")
          ) {
            let objColour;
            ...
            transcriptBits.includes("quit") ||
            transcriptBits.includes("stop") ||
            transcriptBits.includes("cancel") ||
            transcriptBits.includes("nothing")
          ) {
            quit = true;
            recognition.stop();
          ...
          display.innerText =
            "interim results: " + event.results[i][0].transcript;
        }
      }
    };
