...
const initPage = () => {
  ...
  const randomString = Math.random()
    .toString(36)
    .substring(7);
  publishAnchor.setAttribute(
    "href",
    `https://github.com/login/oauth/authorize?client_id=${GITHUB_CLIENT_ID}&scope=public_repo&state=${randomString}&redirect_uri=${window.origin}/studio/pages/publish-confirm/index.html`
  );
...
const handleDownload = event => {
  const fullMarkerImage =
    window.session.fullMarkerImage || window.session.markerImage;
